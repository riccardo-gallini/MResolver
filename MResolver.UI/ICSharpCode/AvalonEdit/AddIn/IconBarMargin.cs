...
namespace ICSharpCode.AvalonEdit.AddIn
{
	...
	public class IconBarMargin : AbstractMargin, IDisposable
	{
        ...
        public static IImage DefaultBookmarkImage  {
            get
            {
                return new ResourceImage("Icons\\ToggleBreakpoint.png");
            }
        }
			...
			if (textView != null && textView.VisualLinesValid) {
				...
				foreach (VisualLine line in textView.VisualLines) {
					...
					if (bookmarkDict.TryGetValue(lineNumber, out bm)) {
						...
						drawingContext.DrawImage((bm.Image ?? DefaultBookmarkImage).ImageSource, rect);
						if (dragDropBookmark == bm && dragStarted)
							drawingContext.Pop();
					}
				}
				if (dragDropBookmark != null && dragStarted) {
					...
					drawingContext.DrawImage((dragDropBookmark.Image ?? DefaultBookmarkImage).ImageSource, rect);
				}
			}
		}
		...
		protected override void OnMouseUp(MouseButtonEventArgs e)
		{
			...
			if (!e.Handled && line != 0) {
				...
				if (e.ChangedButton == MouseButton.Left && TextView != null) {
                    ...
                    var bk = new RBookmark(line, manager);
                    manager.Bookmarks.Add(bk);
